...
function AviaryInfoCard({close, info, progressBarValue}) {
    const [_progressBarValue, setProgressBarValue] = React.useState([{completed: 75}])
    ...
    return (
        <div className='aviary-background'>
            <div className='aviary-info-flex-row'>
                <svg onClick={close} className='close'>
                    <path
            ...
            <div className='percentage-bar-flex-row'>
                {_progressBarValue.map((item, idx) => (
                    <ProgressBar key={idx} completed={item.completed}/>
                ))}

                <div onClick={info} className='info-button'>Информация</div>
            </div>
        </div>
    )
}
